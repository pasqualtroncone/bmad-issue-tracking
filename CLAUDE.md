...
| code-review | Find story with status `review`, switch to worktree | Commit + push + post review + MR + CI gate + optional merge; **both** gating phases ensure the MR first, for the same reason dev-story does — review-finish did not until #95, and it is the only phase bmad-loop ever reaches; the verdict is the spec `status` when the producer set one, else `sprint-status.yaml` (see "which producer wrote the review VERDICT") | story → PRD |
...
- `common/mark-mr-ready.yaml` — no-op when no MR exists (bmad-loop has none). The CI gate DOES run in this flow: **both** gating phases — dev-finish AND review-finish — ensure the trace MR first (`common/ensure-mr.yaml`) and only then gate on it (`common/check-mr-ci.yaml`, `common/wait-for-green-ci.yaml`), so the gate has something to read on the first pass. review-finish did not until #95, and it is the phase that matters here: `bmad-build-auto` finalises dev and review in ONE session, so the hook fires once and routes straight to review-finish — `find-mr` answered nothing, `check-mr-ci` mapped `no_mr`, `write-ci-status` wrote green and bmad-loop's `[verify]` passed on a gate that had read no pipeline, over a branch that had one. `ensure-mr` is a no-op when the MR already exists, so the phases that inherit one are unchanged.
- `ci-status.json` is the gate's OUTPUT, not an input: setup step 4 gitignores it (never `worktree_seed`s it). Untracked it rode bmad-loop's single story commit into the integration branch (#96).
...
- `make -C tests/e2e e2e-static` — level 0, greps only, no lab (the `Makefile` lives in
  `tests/e2e/`, next to the scripts it wraps, and `make test` there runs the offline suite).
  `tests/e2e/replay.sh <case>|all|gitlab` — level 1, literal RUN replays (no LLM);
  `tests/e2e/scenarios/A*.sh` — level 2, headless Claude (50–60 turns of the quota window each).
...
A fifth rule is not checked by `tests/` but by the e2e level-0 report (`make -C tests/e2e e2e-static`,
