...
- **`common/wait-for-green-ci.yaml`** — the on_complete hook blocks until the MR/PR pipeline is green, for up to 30 minutes. The initial check goes through `common/check-mr-ci.yaml` → `common/get-mr-pipeline.yaml`; the rounds that follow inline the same lookup, and `common/get-failed-jobs.yaml` is read only on the failure path. The wait is a `LOOP` of eighteen rounds, each a single command of at most four polls 25 s apart (~100 s, inside the tool's 120 s default): the interpreter runs commands through a tool that times out, so one 30-minute command would be killed before it could report anything and `ci-status.json` would never be written.
  ...
  Every lookup is pinned to the commit the hook pushed (`git rev-parse HEAD`, filtered on as `--commit` on GitHub and as the pipeline's `sha` on GitLab): both platforms list the previous commit's run for the first seconds after a push, so an unpinned gate could report green over a tree CI never built. "No run for this commit yet" is an empty listing, which the rules above already treat as "still coming".
...
- **`ci-status.json`** — transient gate output at the worktree root: one story's verdict, written by `common/write-ci-status.yaml` and read by `ci-status.sh`. The setup skill gitignores it (step 4) because bmad-loop commits the story worktree as a single commit, and untracked the file rode that commit into the integration branch, leaving every story's stale verdict there.
...
| `bmad-build.toml` | `bmad-build` | `on_complete` | Manual flow: routes to the interactive dispatcher, which reads the spec status and runs the matching phase of `post-dev-complete` (push, wait for CI, update issue, comment); offers the optional MR merge on review-finish |
| `bmad-build-auto.toml` | `bmad-build-auto` | `on_complete` | bmad-loop flow (unattended): routes to the non-interactive dispatcher; same phases plus `ci-status.json` for the bmad-loop `[verify]` gate; never prompts and never stages orchestrator-owned files |
...
1. `bmad-loop run` — each story is implemented/reviewed/verified in its own worktree and merged back locally. At the end of every `bmad-build-auto` session, the skill executes its `on_complete` hook (from `bmad-build-auto.toml`), which runs `common/post-build-dispatch-auto.yaml` → `common/post-build-dispatch.yaml` → `common/post-dev-complete.yaml`. This unified workflow handles the full lifecycle for the story:
   ...
   - **review-finish phase** (spec status `done`): commits review modifications, pushes, posts the review findings comment, mirrors the story to its issue (status label, result comment, MR link), then ensures the trace MR, waits for CI and writes `ci-status.json`. It ensures the MR for the same reason dev-finish does, and it is the phase where that matters most: `bmad-build-auto` finalises dev *and* review in one session, so under bmad-loop review-finish is the only phase a story ever reaches — without the MR the gate read nothing and wrote green.
