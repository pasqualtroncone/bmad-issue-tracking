...
| `d38` | 1 | #95 | local: the `review_status eq "done"` gate block of `post-dev-complete.yaml` — the phase bmad-loop reaches, since `bmad-build-auto` finalises dev and review in one session. `common/ensure-mr` must be INCLUDEd inside that same block BEFORE `wait-for-green-ci` and `write-ci-status`; with the gate first, `find-mr` answers nothing, `check-mr-ci` maps `no_mr` and the green `ci-status.json` passes bmad-loop's `[verify]` over a pipeline nobody read. `no_mr` must still map to green in `write-ci-status` |
