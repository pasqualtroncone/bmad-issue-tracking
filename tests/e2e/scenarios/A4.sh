...
#   run 1 (no PRD issue yet, CLEAN tree): regression check for D07 — the create branch
#          commits with `--allow-empty`, so a clean tree must NOT stop the hook before
#          push/MR the way a bare `git commit -m` did.
...
WT="$(prd_worktree)"
# the commit step is located, never spelled out
COMMITL="$(grep -n 'git commit --allow-empty' "$WF/bmad-prd/complete.yaml" | head -1 | cut -d: -f1)"
...
if saw "$D1" 'nothing to commit' ; then
  ...
  else verdict $C-D07 CONFIRMED "the commit at bmad-prd/complete.yaml:${COMMITL:-?} exited 1 with 'nothing to commit' on a clean tree; the hook halted: no push, no draft PR for feat/$PRD_KEY/prd. $notes1"; fi
else verdict $C-D07 REFUTED "no 'nothing to commit' in run 1: bmad-prd/complete.yaml:${COMMITL:-?} commits with --allow-empty, so a clean tree does not stop the hook (issue existed? see status-run1.txt / commands.txt). $notes1"; fi
