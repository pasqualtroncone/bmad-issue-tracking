...
verdict $C OBSERVED "pushed=$(ran "$D" 'git push' && echo yes || echo no) improvised=$(rj "$D" 'r.get("improvised")') turns=$(rj "$D" 'r["result"]["num_turns"]'); final: $(tail -c 300 "$D/final.txt" | tr '\n' ' ')"
