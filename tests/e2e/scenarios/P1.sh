...
if [ "$MODE" = create ]; then
  ...
  if [ -n "$KEYED" ]; then
    verdict $C-create BLOCKED "the consumer's $PLANNING/prd.md already carries prd_key '$KEYED', so /bmad-prd takes the update path whatever the answers ask for: one PRD per repository (#90). No skill run: it would only re-run P1 update again. Run create against a consumer with no prd.md, or an unkeyed one"
    exit 0
  fi
...
notes="mode=$MODE PRs=$(gh pr list -R "$REPO_GH" --state all --json number --jq length) improvised=$(rj "$D" 'r.get("improvised")')/$(rj "$D" 'r["bash_commands"]') turns=$(rj "$D" 'r["result"]["num_turns"]'); final: $(tail -c 300 "$D/final.txt" | tr '\n' ' ')"
